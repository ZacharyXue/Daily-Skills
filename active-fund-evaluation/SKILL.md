...
## 话少型经理验证三件套（报告短 ≠ 调仓少，必须量化验证）

经理季报/年报「运作分析」写得很短（<1000B）时，**文字覆盖不了真实调仓**，必须三件套交叉验证：

1. **换手率**：`invest_style_snapshot.turnover.current`（>100% = 高换手，报告必然没写全）。结合 `peer_rank` 看同类分位。
2. **前十大换血率**：从多期报告 PDF 提取「5.3.1 前十名股票投资明细」表格（字段每行一个：序号/代码/名称/数量/市值/占比%），对比 N 期前十大，看存活率。高换手下前十大每季换一大半=正常。
3. **调仓成本与效果**：
   - 成本：从中报/年报 PDF「10.7 券商交易单元」表提取股票成交总额+佣金 → 佣金费率（佣金改革后应 <0.05%）+ 佣金占净值比。高换手+低佣金+小规模 = 摩擦可控。
   - 效果：`cash_flow_and_performance_history` 的 SE（单季收益）对照同期沪深300估值；再对照长期 rank（近1/3/5年），确认换手换来了超额还是白折腾。

**判读关键词**：报告短但换手高+前十大换血快 = 说少做多型（言行层面看行业方向表达对不对，别看个股）；高换手若同时长期 rank 靠后 = 无效折腾（副作用）。小规模（<10亿）+高换手可接受，规模长大要警惕冲击成本侵蚀超额。

若要完整覆盖：报告篇幅、换手率、前十大存活率、佣金净值占比、单季胜率、长期 rank——全查一遍才算「验证过调仓」。实测案例：南方核心竞争（202213）换手132%、佣金0.07%/半年、单季5/6跑赢、近5年rank290 = 有效换手。
