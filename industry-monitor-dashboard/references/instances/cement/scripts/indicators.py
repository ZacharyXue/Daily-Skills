...
INDICATORS = [
    ...
    dict(id="payout", group="财务安全垫组", name="分红率", unit="%", ttl="quarter",
         ...
         getter="payout_rate"),
    # ---- 人工采集指标（自动源拿不到，见 prompts/collect_prompts.md；fetch 读 manual.json 覆盖为 manual）----
    dict(id="po425_east", group="价格组", name="华东P.O42.5均价", unit="元/吨", ttl="week",
         source="人工采集(数字水泥网)", source_url="https://data.ccement.com", getter="manual",
         meaning="华东主战场区域价，验证价格修复是否区域性。",
         signal="华东≥300且连涨=区域止跌；贴近成本线(≈255)=磨底"),
    dict(id="po425_south", group="价格组", name="华南P.O42.5均价", unit="元/吨", ttl="week",
         source="人工采集(数字水泥网)", source_url="https://data.ccement.com", getter="manual",
         meaning="华南海螺市占率高，验证南方价格。",
         signal="华南≥300=修复；粤桂≈260/含琼316"),
    dict(id="electricity", group="成本组", name="安徽工业电价", unit="元/千瓦时", ttl="month",
         source="人工采集(安徽省发改委)", source_url="https://www.ah.gov.cn", getter="manual",
         meaning="成本组：电价稳则煤价主导成本弹性。",
         signal="电价不上调=成本端无额外压力(≈0.64)"),
    dict(id="capacity_util", group="量组", name="全国水泥产能利用率", unit="%", ttl="quarter",
         source="人工采集(中国水泥大数据)", source_url="https://www.ccement.com", getter="manual",
         meaning="供给端：产能利用率升=需求回暖或供给收缩。",
         signal="从≈48%升到55-60%+=止跌回稳"),
    dict(id="housestart", group="量组", name="地产新开工同比", unit="%", ttl="month",
         source="人工采集(国家统计局)", source_url="https://www.stats.gov.cn", getter="manual",
         meaning="水泥需求先行：新开工降幅收窄=需求止跌。",
         signal="新开工同比收窄(如-23%→-10%)=需求企稳"),
    dict(id="housestart2", group="量组", name="地产施工面积同比", unit="%", ttl="month",
         source="人工采集(国家统计局)", source_url="https://www.stats.gov.cn", getter="manual",
         meaning="施工面积决定存量需求。",
         signal="施工降幅收窄=存量需求止跌(当前-12.5%)"),
    dict(id="infra_invest", group="量组", name="基建投资同比", unit="%", ttl="month",
         source="人工采集(国家统计局)", source_url="https://www.stats.gov.cn", getter="manual",
         meaning="基建对冲地产下滑。",
         signal="基建提速=水泥需求支撑(1-5月+0.6%,Q1+8.9%)"),
    dict(id="special_bond", group="量组", name="专项债发行进度", unit="%", ttl="month",
         source="人工采集(财政部)", source_url="https://www.mof.gov.cn", getter="manual",
         meaning="专项债提速=基建资金到位。",
         signal="进度>50%=资金支撑(H1 47%,2.07万亿)"),
    dict(id="cr10", group="供给/出清组", name="水泥CR10集中度", unit="%", ttl="year",
         source="人工采集(大公国际)", source_url="https://www.dagong.com", getter="manual",
         meaning="软出清核心：CR10上行=小厂退出、龙头整合。",
         signal="CR10上行(>65%且升)=出清推进"),
    dict(id="helluo_share", group="供给/出清组", name="海螺市占率", unit="%", ttl="year",
         source="人工采集(券商研报)", source_url="https://data.eastmoney.com", getter="manual",
         meaning="龙头份额提升=软出清中的赢家。",
         signal="18%→20%+=海螺份额提升；实际14-15.6%"),
]
