...
> 区别于自建 skill（见根目录 README.md 索引）——这里的 skill 来自官方安装/开源项目，文件实体保留在 `~/.hermes/skills/` 原位置。

## 当前记录

### 1. 天天基金（ttskill-cli）

- **位置**：`~/.hermes/skills/research/ttskill-cli/`
- **用途**：查询基金持仓、收益、季报；fund analysis（profit_contribute 看盈亏、TTFUND_BASE_INFOS 看持仓/风格）
- **说明**：天天基金官方 CLI，安装时通过 Hermes agent-entry + OpenCode 分发
...
- **位置**：`~/.hermes/skills/note-taking/weread-reading-notes/`
- **用途**：从微信读书提取划线/想法 → 整理为结构化读书笔记 → 逐章与用户确认 → 发布到博客
- **说明**：基于微信读书 Agent API（Tencent/WeChatReading skill v1.0.4）
...
遇到重要的外部/开源 skill 时，在此文件追加条目（名称、位置、用途、说明）。
不要移动文件到本仓库——zach-skills 只收录自建 skill。