...
- **⚠️ `--check` 只验 cookie，别拿它判断风控**：`xueqiu_login.py --check` 的 verify_login 只能在请求**到达服务器**时验登录态；IP 被 WAF 拦截时请求根本没到雪球，`--check` 可能仍报「✓ 有效」。同理取数报「登录态失效（10022）」可能是**误报**（HTML 挑战页被 check_login 当成 10022），实际是 IP 风控。判别方法：看下层 `xueqiu_fetch` 的**原始响应体**——`<!DOCTYPE html>` = IP 风控（等冷却）；JSON 含 `error_code:10022` = 真登录态失效（重新扫码）
- **风控分层参考（外部情报）**：L1 软限流(429/验证码, 单IP>10-30次/分触发)；L2 临时封禁(403/空白页, 15min-72h 自动放)；L3 IP段/机房封禁(云厂商段被整段标记, 等数天-两周或弃段)；L4 账号/Cookie 连带风控(带 xq_a_token 爬可能连账号被限)。**带登录态爬 = L4 风险存在**，解封后先用手机流量/宽带无痕窗口验证是 IP 层还是账号层（网页能开=原 IP 被封，同网络也打不开=可能账号层）
- **解封 ≠ 原样重爬**：协议禁止爬虫/AI 语料抓取。恢复后保持个人低频查看（15-60s/页、单会话≤50请求、拆多轮），只做摘要不建库不训练；批量整库/训练属红线
