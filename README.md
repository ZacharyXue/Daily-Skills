...
薛子皓的自建 Hermes skill 集。所有 skill 遵循 Hermes skill 规范（YAML frontmatter + Markdown），通过软链接入 `~/.hermes/skills/`。
...
| [skill-creation-guide](skill-creation-guide/) | 创建/管理自建 skill 的完整流程：规范、目录结构、软链接入、README 维护 | 新建 skill、修改自建 skill、维护本仓库 |

最新列表以 [git log](https://github.com/ZacharyXue/Daily-Skills/commits/main) 为准；本 README 每新增一个自建 skill 必须同步更新。

## 仓库结构

```
zach-skills/
├── README.md                    ← 本文件：自建 skill 说明 + 下载方式
├── OPENSOURCE_SKILLS.md         ← 当前 Hermes 使用的全部开源 skill 清单（+manifest hash）
└── <skill-name>/                ← 每个自建 skill 一个目录（SKILL.md + references/ + scripts/）
```

## 开发规范

- 每个自建 skill 必须有：`name`（小写连字符）、`description`（含触发时机）、`version`
- 新建 skill 后创建软链：`ln -sfn /root/zach-skills/<name> ~/.hermes/skills/<name>`
- 修改自建 skill 直接改仓库文件（**不要用 skill_manage 操作软链**，会失败），改完 `git add + commit`
- 代码只 commit 不 push，推送由用户自行决定
- 新增/删除自建 skill 必须同步更新本 README 索引表

## 相关

- Hermes 官方文档：https://hermes-agent.nousresearch.com/docs
- 开源 skill 审计：见 [OPENSOURCE_SKILLS.md](OPENSOURCE_SKILLS.md)