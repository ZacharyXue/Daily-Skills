...
## 提醒节奏（⚠️ 现状：2026-09-11 起 cron 全部停用，下表仅存档）

> ⚠️ 2026-09-11：cron 已全部删除（systemd + 容器无 userbus 环境问题），当前提醒**靠会话内触发词**（「看 todo / 记个问题 / 开始深挖 / 复盘」）。
> 脚本保留：需要时手动 `python3 scripts/remind_daily.py` 验证输出内容。
